...
A:
  py2 = random(-200);
  if (abs(py1 - py2) > 100) goto A;

B:
  py3 = random(-200);
  if (abs(py2 - py3) > 100) goto B;

C:
  py4 = random(-200);
  if (abs(py3 - py4) > 100) goto C;
}
...
void chk_pipe() {
  ...
  {
    if (getpixel(x, yt - 1) == (GREEN) || getpixel(x, yb + 1) == (GREEN) ||
        getpixel(x, yt - 1) == BROWN || getpixel(x, yb + 1) == BROWN ||
        (yt - 1) < 0 || (yb + 1) > getmaxy() || getpixel(x, yt - 1) == RED ||
        getpixel(x, yb + 1) == RED) {
      clash = 1;
      rem_heli();
    }
  }
  if (getpixel(x, yb) == (GREEN) || getpixel(x, yt) == (GREEN) ||
      getpixel(x, yb) == BROWN || getpixel(x, yt) == BROWN ||
      getpixel(x, yb) == (RED) ||
      getpixel(x, yt) == (RED))  // chk for front obstacle
  {
    clash = 1;
    rem_heli();
  }
...
void drawMenu(int selected, int defCol, int selCol) {
  ...
  int i;
  int TotalOptions = 4;
  char menu_option[4][14] = {" PLAY     ", " LEVELS ", " ABOUT ", " EXIT     "};
...
void level_draw(int lx, int ly) {
  ...
  switch (level) {
    case 1:
      bar(lx, ly, lx + 300, ly + 260);
      bar(getmaxx() / 2 - 50, getmaxy() - 50, getmaxx() / 2 - 40,
          getmaxy() - 40);
      break;
    case 2:
      bar(lx + 350, ly, lx + 650, ly + 260);
      bar(getmaxx() / 2 - 20, getmaxy() - 50, getmaxx() / 2 - 10,
          getmaxy() - 40);
      break;
    case 3:
      bar(lx + 700, ly, lx + 1000, ly + 260);
      bar(getmaxx() / 2 + 10, getmaxy() - 50, getmaxx() / 2 + 20,
          getmaxy() - 40);

      break;
    case 4:
      bar(lx + 1050, ly, lx + 1350, ly + 260);
      bar(getmaxx() / 2 + 40, getmaxy() - 50, getmaxx() / 2 + 50,
          getmaxy() - 40);

      break;
    default:
      cout << "\n\nThere is some error in the game....CONTACT DEVELOPERS "
              "(Karan Khanna and Ayushi Jain)";
  }
